...
class Model(Resource):
    def get(self):
        ...
        try:
            ...
            return {"word" : args['word'], 'rep':res.tolist()}
        except Exception(e):
            print(e)
            return
...
@app.route('/tasks/add/<param1>/<param2>')
def add(param1, param2):
    task =  celery.send_task('tasks.add',args=[param1,param2], kwargs={})
    response = "<a href ='{}'>check status of {}</a> ".format(
      url_for('check_task', task_id=task.id, external =True),
        task.id
    )
    return response
